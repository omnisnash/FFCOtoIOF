...
/**
 * 
 *                 Used to state start time allocation requests. It consists of a possible reference Organisation or Person
 *                 and the allocation request, e.g. late start or grouped with the reference Organisation/Person. This way
 *                 it is possible to state requests to the event organizer so that e.g. all members of an organisation has
 *                 start times close to each other - or parents have start times far from each other. It is totally up to
 *                 the event software and organizers whether they will support such requests.
 *             
 * 
